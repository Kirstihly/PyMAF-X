...
def prepare_rendering_results(person_data, nframes):
    ...
    for idx, frame_id in enumerate(person_data["frame_ids"]):
        ...
        frame_results[frame_id][person_id] = {
            ...
            "smplx_verts": person_data["smplx_verts"][idx]
            if "smplx_verts" in person_data
            else None,
            "cam": person_data["orig_cam"][idx],
            "cam_t": person_data["orig_cam_t"][idx]
            if "orig_cam_t" in person_data
            else None,
...
def run_demo(args):
    ...
    if args.image_folder is None:
        ...
        img_shape = cv2.imread(
            osp.join(image_folder, os.listdir(image_folder)[0])
        ).shape
    ...
    checkpoint_paths = {
        "body": args.pretrained_body,
        "hand": args.pretrained_hand,
        "face": args.pretrained_face,
    }
    ...
    if not all(
        [
            args.pretrained_body is None,
            args.pretrained_hand is None,
            args.pretrained_face is None,
        ]
    ):
    ...
    if args.recon_result_file:
        ...
        dataloader = DataLoader(
            dataset, batch_size=args.model_batch_size, num_workers=16
        )
        ...
        print(
            f"Total time spent for reconstruction: {total_time:.2f} seconds (including model loading time)."
        )
    ...
    if not args.no_render:
        ...
        output_img_folder = os.path.join(
            output_path, osp.split(image_folder)[-1] + "_output"
        )
        ...
        for frame_idx in tqdm(range(len(image_file_names))):
            ...
            if args.render_ratio != 1:
                img = resize(
                    img,
                    (
                        int(img.shape[0] * args.render_ratio),
            ...
            if args.vid_file is not None:
                imsave(
                    os.path.join(output_img_folder, f"{frame_idx:06d}.png"), img_full
                )
                imsave(
                    os.path.join(output_img_folder, "arm", f"{frame_idx:06d}.png"),
                    img_arm,
                )
            else:
                imsave(
                    os.path.join(
                        output_img_folder, osp.split(img_fname)[-1][:-4] + ".png"
                    ),
                    img_full,
                )
                imsave(
                    os.path.join(
                        output_img_folder, "arm", osp.split(img_fname)[-1][:-4] + ".png"
                    ),
                    img_arm,
                )
        ...
        if args.vid_file is not None:
            vid_name = (
                ...
                else os.path.basename(video_file)
            )
            ...
            images_to_video(
                img_folder=output_img_folder + "/arm",
                output_vid_file=save_name.replace(".mp4", "_arm.mp4"),
            )
            images_to_video(
                img_folder=image_folder,
                output_vid_file=save_name.replace(".mp4", "_raw.mp4"),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img_file", type=str, default=None, help="Path to a single input image"
    )
    parser.add_argument(
        "--vid_file", type=str, default=None, help="input video path or youtube link"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--single_person", action="store_true", help="only one person in the scene."
    )
    parser.add_argument(
        "--cfg_file",
        type=str,
    ...
    parser.add_argument(
        "--pretrained_model", default=None, help="Path to network checkpoint"
    )
    parser.add_argument(
        "--pretrained_body",
        default=None,
    ...
    parser.add_argument(
        "--misc", default=None, type=str, nargs="*", help="other parameters"
    )
    parser.add_argument(
        "--model_batch_size", type=int, default=8, help="batch size for SMPL prediction"
    )
    parser.add_argument(
        "--display",
        action="store_true",
    ...
    parser.add_argument(
        "--render_part", type=str, default="arm", help="render part mesh"
    )
    parser.add_argument(
        "--render_model",
        type=str,
    ...
    parser.add_argument(
        "--empty_bg", action="store_true", help="render meshes on empty background."
    )
    parser.add_argument(
        "--use_gt",
        action="store_true",
    ...
    parser.add_argument(
        "--anno_file", type=str, default="", help="path to tracking annotation file."
    )
    parser.add_argument(
        "--render_ratio", type=float, default=1.0, help="ratio for render resolution"
    )
    parser.add_argument(
        "--recon_result_file",
        type=str,
    ...
    parser.add_argument(
        "--pre_load_imgs", action="store_true", help="pred-load input images."
    )
    parser.add_argument(
